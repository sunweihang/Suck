"""Cut the 3D gold coin off a dark plate and publish ui-gold-icon.png."""
...
SRC = ROOT / "tools" / "ai-item" / "gold-coin-3d.png"
OUT = ROOT / "assets" / "resources" / "ui" / "ui-gold-icon.png"
SIZE = 256
PAD = 20
...
def knockout(rgb: np.ndarray) -> np.ndarray:
    ...
    gold = (r > 120) & (g > 70) & (r >= g) & (sat > 0.16)
    dark = (lum < 48) & (sat < 0.24)
    ...
    fringe = dark & (~bg) & (~gold)
    fade = np.clip((48.0 - lum) / 48.0, 0.0, 1.0)
    alpha = np.where(fringe, alpha * (1.0 - fade), alpha)
    leftover = (~bg) & (~gold) & (lum < 32)

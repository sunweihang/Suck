...
function main() {
  ...
  const root = addNode(doc, { name: 'FailPanel' });
  addPrefabInfo(doc, root, { __id__: 0 }, true);
  addUITransform(doc, root, 1080, 1920);
  addWidget(doc, root);
  addScript(doc, root, UUID.FailPanel);
    ...
    `${JSON.stringify(
      {
        ...
        files: ['.json'],
        subMetas: {},
        userData: { syncNodeName: 'FailPanel' },
      },
      null,
      2,
    )}\n`,
  ...
  console.log(
    `wrote ${path.relative(ROOT, OUT)} panel=${PANEL_W}x${PANEL_H} retry=${RETRY_W}x${RETRY_H} y=${BTN_Y}`,
  );
}

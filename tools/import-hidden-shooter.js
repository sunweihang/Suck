...
/** Original unactivated turret: Shooter_Hidden + T_Hidden_Pattern. */
const fs = require('fs');
...
const OBJ = path.join(SRC, 'meshes', 'Shooter_Hidden.obj');
const TEX = path.join(SRC, 'textures', 'T_Hidden_Pattern.png');

const UUID = {
  MeshJson: '7e22bb20-030d-4b02-8002-00000000000d',
  TexImg: '9d16cc10-0403-4a01-8001-000000000004',
};
...
function main() {
  ...
  fs.writeFileSync(path.join(meshDir, 'hidden-shooter.json'), `${JSON.stringify(mesh)}\n`);
  fs.writeFileSync(
    path.join(meshDir, 'hidden-shooter.json.meta'),
    `${JSON.stringify({
      ver: '2.0.1',
      importer: 'json',
      imported: true,
  ...
  const texDst = path.join(ROOT, 'assets', 'resources', 'toys', 'hidden-pattern.png');
  fs.copyFileSync(TEX, texDst);
  fs.writeFileSync(`${texDst}.meta`, `${JSON.stringify(imageMeta(UUID.TexImg, 'hidden-pattern'), null, 2)}\n`);
  console.log('hidden-shooter verts', mesh.p.length / 3, 'tris', mesh.i.length / 3);

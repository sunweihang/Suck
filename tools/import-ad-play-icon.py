#!/usr/bin/env python3
"""AI ad-play icon → transparent 256 PNG + Cocos meta."""
...
SRC = Path(
    "/Users/CreativeCenter/Suck/tools/ai-item/ad-choices/ic-ad-choice-b.png"
)
WORK = ROOT / "tools/ai-item"
OUT = ROOT / "assets/resources/ui/ic-ad-play.png"
UUID = "7e22bb20-009d-4b02-8002-00000000009d"
SIZE = 1024
OUTLINE = (92, 36, 120, 255)
OUTLINE_PX = 42
...
def flood_alpha(rgb: Image.Image) -> Image.Image:
    ...
    out.putalpha(Image.fromarray(alpha, "L").filter(ImageFilter.GaussianBlur(1.2)))
    return out
...
def fit_padded(im: Image.Image, max_side: int) -> Image.Image:
    bbox = im.getbbox()
    if not bbox:
        return Image.new("RGBA", (max_side, max_side), (0, 0, 0, 0))
    crop = im.crop(bbox)
    pad = int(round(max(crop.size) * 0.03))
    w, h = crop.size[0] + pad * 2, crop.size[1] + pad * 2
...
def add_outline(im: Image.Image, radius: int, color: Tuple[int, int, int, int]) -> Image.Image:
    alpha = im.split()[3]
    odd = radius * 2 + 1
    ring = alpha.filter(ImageFilter.MaxFilter(odd))
    base = Image.new("RGBA", im.size, (0, 0, 0, 0))
    plate = Image.new("RGBA", im.size, color)
    base.paste(plate, (0, 0), ring)
...
def main() -> None:
    ...
    studio = WORK / "ic-ad-choice-b-studio.png"
    shutil.copy2(SRC, studio)
    cache = WORK / "ic-ad-choice-b.rmbg.png"
    ...
    if cache.exists():
        ...
        try:
            ...
            cut = flood_alpha(src)
            cut.save(cache, "PNG")
    out = fit_padded(apply_alpha(src, cut), SIZE)
    OUT.parent.mkdir(parents=True, exist_ok=True)
    out.save(OUT, "PNG")
    write_meta(OUT, UUID, out.size[0], out.size[1])

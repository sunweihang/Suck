...
const KEEP_OFF_RIG = new Set(['Power', 'Rig', 'LockNails', 'BlobShadow']);
...
export class OctopusQAnim {
  ...
  aimAt(_world: Vec3): void {
    /* sit upright; do not turn toward shots */
  }
  ...
  tick(dt: number, state: AnimState, inflight = 0): void {
    ...
    if (!this._eyeL) {
      ...
      const sx = clamp(1 + breath * amp + side * amp * 0.18 + this._sx + spit * 0.04, 0.86, 1.2);
      const sy = clamp(1 - breath * amp * 1.15 + this._sy - spit * 0.05, 0.86, 1.2);
      const sz = clamp(1 + breath * amp * 0.55 - side * amp * 0.12 + this._sz + spit * 0.08, 0.86, 1.2);
      rig.setScale(sx, sy, sz);
      rig.setRotationFromEuler(this._rx * 0.45, this._ry * 0.3, this._rz * 0.45);
      return;
    }

...
export class VictoryPanel extends Component {
  ...
  show(opts?: { hasNext?: boolean; gold?: number; canDouble?: boolean }): void {
    this._ensureTree();
  ...
  private _placeRow(): void {
    ...
    const next = card.getChildByName('NextBtn');
  ...
  private _ensureBtns(): void {
    ...
    const next = card.getChildByName('NextBtn') ?? this._mk('NextBtn', card, BTN_W, BTN_H);
  ...
  private _paintFrame(): void {
    ...
    applyArtSpriteSoon(card.getChildByName('Frame'), 'winPanel', CARD_W, CARD_H);
  }

  private _paintBtns(): void {
    const card = this._card();
    const double = card?.getChildByName('DoubleBtn') ?? null;
    const next = card?.getChildByName('NextBtn') ?? null;
    ...
    const nLab = card?.getChildByName('NextBtn')?.getChildByName('Label');
    if (dLab) this._styleLabel(dLab, '双倍领取', DOUBLE_OUTLINE);
    if (nLab) this._styleLabel(nLab, '下一关', NEXT_OUTLINE);
    applyArtSpriteSoon(
  ...
  private _bindEvents(): void {
    this._bindTap(this._card()?.getChildByName('NextBtn'), () => {

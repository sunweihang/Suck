...
export class VictoryPanel extends Component {
  ...
  show(_opts?: { hasNext?: boolean }): void {
    this._ensureTree();
    this.node.active = true;
    const next = this._card()?.getChildByName('NextBtn');
    if (next) {
      next.active = true;
      next.setPosition(0, next.position.y, 0);
    }
    this.layoutChrome();
    this._popIn();
  }
  ...
  private _bindEvents(): void {
    this._bindTap(this._card()?.getChildByName('NextBtn'), () => this._onNext?.());
  }

...
export class ExportContext {
  ...
  constructor(graph: NodeGraphJSON, abilityId: number) {
    this.graph = graph;
  ...
  getFlowCallsFromEntrance(portIndex: number): string[] {
    const entrance = this.findEntrance();
    if (!entrance) return [];
    return this.formatFlowCallLines(entrance.id, portIndex);
  }

  formatFlowCalls(nodeId: string, flowOutIndex: number, indent = '        '): string {
    const lines = this.formatFlowCallLines(nodeId, flowOutIndex);
    if (lines.length === 0) return '';
    return lines.map((l) => indent + l).join('\n') + '\n';
  }

  private formatFlowCallLines(nodeId: string, absoluteOutPortIndex: number): string[] {
  ...
  private dataPreludeLines(node: NodeDataJSON): string[] {
    const lines: string[] = [];
    ...
    for (const id of need) {
      ...
      if (v) lines.push(`this.${v.methodName}();`);
    }
    return lines;
  }
  ...
  private hasIncomingFlowConnection(nodeId: string): boolean {
    const node = this.findNode(nodeId);
    if (!node) return false;
    for (let i = 0; i < node.inputs.length; i++) {
      if (node.inputs[i].portType !== FLOW) continue;
      if (this.inConns(nodeId, i).length > 0) return true;
    }
    return false;
  }

  buildNodePlaceholders(info: NodeVisitInfo): Record<string, string> {
    ...
    const def = SKILL_NODE_DEFS.find((d) => d.typeName === node.typeName);
    if (def?.fields) {

...
import { validateGraphJSON } from '../validateSkillGraph';
import { syncGraphPortsFromDefs } from '../syncSkillGraphPorts';
...
export function exportSkillTs(skillId: number): ExportResult {
  const graphText = readFsText(graphFsPath(skillId));
  if (!graphText) {
    return { ok: false, skillId, error: `找不到图: ${graphFsPath(skillId)}` };
  }
  ...
  if (syncGraphPortsFromDefs(graph)) {
    writeFsText(graphFsPath(skillId), JSON.stringify(graph, null, 2));
  }

  const validation = validateGraphJSON(graph, skillId);
  if (!validation.ok) {
  ...
  if (!entrance) {
    return { ok: false, skillId, error: `缺少 ${ENTRANCE_TYPE}` };
  }

  const ctx = new ExportContext(graph, skillId);
  ctx.collectReachable(entrance);

  const classTpl = loadTemplate('AbilityClass.ts.tpl');
  const entranceTpl = loadTemplate('AbilityEntranceBlueprint.ts.tpl');
  ...
  const className = `${CLASS_PREFIX}${skillId}`;
  ...
  const outPath = generatedClassFsPath(skillId);
...
export function exportAllFlagged(): { ok: number; fail: number; results: ExportResult[] } {
  const skills = listLocalSkills().filter((s) => s.exportFlag !== false);
...
function touchExportFlag(skillId: number): void {
  const p = indexFsPath(skillId);
  if (!fs.existsSync(p)) return;
  try {
    const index = JSON.parse(fs.readFileSync(p, 'utf8')) as SkillIndexJSON;

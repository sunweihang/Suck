...
import { listLocalSkills, SkillListItem } from '../browseSkills';
...
function renderList(items: SkillListItem[]): void {
  if (!listEl) return;
  listEl.innerHTML = '';
  if (items.length === 0) {
  ...
  listEl.querySelectorAll('button').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      if (act === 'edit') {
        ...
        const r = validateSkillOnDisk(id);
        const msg = r.ok
          ? `校验通过\n${r.warnings.join('\n')}`
          : `校验失败\n${r.errors.join('\n')}\n${r.warnings.join('\n')}`;
        try {

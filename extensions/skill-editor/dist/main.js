...
exports.openSkill = openSkill;
exports.load = load;
exports.unload = unload;
const browseSkills_1 = require("./browseSkills");
const createSkill_1 = require("./createSkill");
const TsAbilityExporter_1 = require("./export/TsAbilityExporter");
const skillNodes_1 = require("./nodes/skillNodes");
const skillPortTypes_1 = require("./nodes/skillPortTypes");
const paths_1 = require("./paths");
const profile_1 = require("./profile");
const reloadExtension_1 = require("./reloadExtension");
...
async function skillNodesMissingFromRegistry() {
    try {
        const defs = (await Editor.Message.request(NODE_GRAPH, 'query-node-defs'));
        const names = new Set((defs !== null && defs !== void 0 ? defs : []).map((d) => d === null || d === void 0 ? void 0 : d.typeName).filter(Boolean));
...
async function ensureRegistered(silent = false) {
    if (!(await ensureNodeGraph(silent)))
        return { ok: false };
    if (registered && !(await skillNodesMissingFromRegistry()))
        return { ok: true };
    try {
        const nodes = (0, skillNodes_1.allSkillRegisterNodes)();
        await Editor.Message.request(NODE_GRAPH, 'register-port-types', { portTypes: skillPortTypes_1.SKILL_PORT_TYPES });
        await Editor.Message.request(NODE_GRAPH, 'register-nodes', { nodes });
        registered = true;
        console.log(`[skill-editor] registered ${nodes.length} skill nodes (${skillNodes_1.SKILL_NODE_DEFS.length} domain + builtins)`);
        return { ok: true };
    }
    catch (e) {
        registered = false;
        if (silent)
            console.warn('[skill-editor] register failed', e);
        else
            await dialogError(`注册技能节点失败: ${e}`);
        return { ok: false };
    }
}
...
async function openSkill(arg) {
    const skillId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.skillId;
    if (!skillId) {
    ...
    (0, syncSkillGraphPorts_1.syncSkillGraphOnDisk)(skillId);
    await Editor.Message.request(NODE_GRAPH, 'open-graph', {
        path: (0, paths_1.graphDbUrl)(skillId),
        profile: (0, profile_1.buildSkillGraphProfile)(),
    });
    return { ok: true };
}
async function promptSkillId(defaultId, title) {
...
exports.methods = {
    ...
    async validateSkill(arg) {
        const skillId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.skillId;
        if (!skillId)
            return { ok: false };
        const r = (0, validateSkillGraph_1.validateSkillOnDisk)(skillId);
        const msg = r.ok
            ? `校验通过\n${r.warnings.join('\n')}`
            : `校验失败\n${r.errors.join('\n')}\n${r.warnings.join('\n')}`;
        if (r.ok)
    ...
    async createSkill(arg) {
        ...
        const result = await (0, createSkill_1.createSkillAssets)({
            skillId,
            name: `Skill ${skillId}`,
            exportFlag: true,
        });
    ...
    async createSkillApi(arg) {
        var _a;
        if (!(await ensureRegistered()).ok)
            return { ok: false, error: 'node-graph 未就绪' };
        const skillId = arg === null || arg === void 0 ? void 0 : arg.skillId;
        if (!skillId || skillId <= 0) {
            return { ok: false, error: '请手动指定 skillId' };
        }
        return (0, createSkill_1.createSkillAssets)({
            skillId,
            name: (arg === null || arg === void 0 ? void 0 : arg.name) || `Skill ${skillId}`,
    ...
    async editSkillGraph() {
        if (!(await ensureRegistered()).ok)
            return;
        const skills = (0, browseSkills_1.listLocalSkills)();
        if (skills.length === 0) {
    ...
    async listSkills() {
        return (0, browseSkills_1.listLocalSkills)();
    },
    async validateSkillGraph() {
        ...
        for (const s of skills) {
            const r = (0, validateSkillGraph_1.validateSkillOnDisk)(s.skillId);
    ...
    async exportTs() {
        ...
        const last = skills[skills.length - 1];
        const r = (0, TsAbilityExporter_1.exportSkillTs)(last.skillId);
        if (!r.ok) {
    ...
    async exportSkill(arg) {
        const skillId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.skillId;
        if (!skillId)
            return { ok: false, error: 'missing skillId' };
        const r = (0, TsAbilityExporter_1.exportSkillTs)(skillId);
        if (!r.ok)
            await dialogError(`导出失败: ${r.error}`);
        else
            await dialogInfo(`已导出 ${skillId}\n${r.path}`);
        return r;
    },
    async exportTsBatch() {
        const { ok, fail, results } = (0, TsAbilityExporter_1.exportAllFlagged)();
    ...
    async openSkillDebugScene() {
        ...
        await dialogInfo('已打开正式关卡 Level.scene。\n运行后：\n' +
            'F9 = 开关「点地种怪」\n' +
    ...
    async reloadExtension() {
        try {
            ...
            try {
                await (0, reloadExtension_1.reloadExtensionPackage)(PKG);
                return { ok: true };
            }
            catch (e2) {
    ...
    async rebuildAndReload() {
        try {
            ...
            const built = await (0, reloadExtension_1.buildExtension)();
            if (!built.ok) {

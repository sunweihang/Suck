...
/**
 * 种植可编辑；节奏只读（来自 Excel→Luban→tbspawnconfig）。
 * 选择：TbAvatar 模型资源 → 绑定的 monster/hero 配置（同一模型可多配置）。
 * 保存只写 logic index.json；场景预览直接显示单位模型。
 */

type Vec3 = { x: number; y: number; z: number };
type PlantKind = 'monster' | 'hero';
type Item = {
  unitKind?: PlantKind;
  unitConfigId?: number;
  avatarId?: number;
...
type Layer = { layerId: number; layerName?: string; items: Item[] };
type Bundle = {
...
function numsOrDash(arr?: number[], empty = '—'): string {
...
function isAreaItem(item: Item): boolean {
...
function kindZh(kind: PlantKind): string {
  return kind === 'monster' ? '怪物' : '英雄';
}

function formatPickLabel(avatar: PlantableAvatar, config: PlantableConfig): string {
...
module.exports = Editor.Panel.define({
  ...
  ready(this: {
    ...
    _spawnEditorBound?: boolean;
    _reloadSpawnEditor?: () => void;
    _stopDragSync?: () => void;
  }) {
    const metaEl = this.$.meta as HTMLElement;
    const listEl = this.$.list as HTMLElement;
    const pickerEl = this.$.picker as HTMLElement;
    const pickerListEl = this.$.pickerList as HTMLElement;
    const pickerSearchEl = this.$.pickerSearch as HTMLInputElement;
    ...
    let assetsSceneId = 0;
    let logicName = '';
    let bundle: Bundle = { layers: [] };
    let layerConfigs: LayerConfig[] = [];
    let missingLayerIds = new Set<number>();
    let previewTimer: ReturnType<typeof setTimeout> | null = null;
    let dragSyncTimer: ReturnType<typeof setInterval> | null = null;
    let plantables: PlantableAvatar[] = [];
    let pickerTarget: Item | null = null;
    let pickerExpanded = new Set<number>();
    /** 正在把场景拖拽坐标写回面板，避免触发重建预览 */
    let applyingScenePos = false;

    const close = () => {
      try {
        Editor.Panel.close?.('scene-editor.spawn-editor');
      } catch {
        /* ignore */
      }
    };
    ...
    const configOf = (layerId: number): LayerConfig | null =>
      layerConfigs.find((c) => c.layer_id === layerId) || null;

    const nextId = () => bundle.layers.reduce((m, l) => Math.max(m, l.layerId), 0) + 1 || 1;

    const loadPlantables = async () => {
      try {
        plantables = ((await Editor.Message.request(
          'scene-editor',
          'list-plantables'
        )) as PlantableAvatar[]) || [];
      } catch (e) {
    ...
    const findResolved = (item: Item): { avatar: PlantableAvatar; config: PlantableConfig } | null => {
      if (item.unitKind && item.unitConfigId) {
        for (const avatar of plantables) {
          const config = avatar.configs.find(
            (c) => c.kind === item.unitKind && c.id === item.unitConfigId
          );
    ...
    const defaultPick = (): { avatar: PlantableAvatar; config: PlantableConfig } | null => {
      const preferred =
        plantables.find((a) => a.configs.some((c) => c.kind === 'monster' && c.key === 'Enemy00')) ||
    ...
    const applyPick = (item: Item, avatar: PlantableAvatar, config: PlantableConfig, area: boolean) => {
      ...
      if (config.kind === 'monster') {
        ...
        if (area) {
          ...
          item.enemyCount = Math.max(1, item.enemyCount ?? 1);
        } else {
    ...
    const renderPickerList = () => {
      ...
      for (const avatar of filtered) {
        ...
        row.addEventListener('click', () => {
          ...
          if (pickerExpanded.has(avatar.avatarId)) pickerExpanded.delete(avatar.avatarId);
          else pickerExpanded.add(avatar.avatarId);
          renderPickerList();
        });
        box.appendChild(row);
        ...
        if (expanded || only) {
          ...
          for (const config of avatar.configs) {
            ...
            const isSel =
              selected?.config.kind === config.kind && selected?.config.id === config.id;
    ...
    const flattenToOneItemPerLayer = () => {
      ...
      for (const layer of bundle.layers) {
        ...
        if (!items.length) {
          ...
          if (!configOf(layer.layerId)) newMissing.add(layer.layerId);
          continue;
        }
        items.forEach((it, i) => {
          ...
          if (!configOf(layerId)) newMissing.add(layerId);
        });
      }
    ...
    const refreshScenePreview = (openPrefab = false) => {
      ...
      void (async () => {
        ...
        if (!r?.ok) console.warn('[spawn-editor] 场景预览失败:', r?.reason || 'unknown');
      })();
    };

    const schedulePreview = () => {
      if (applyingScenePos) return;
    ...
    const syncPositionsFromScene = async () => {
      if (!assetsSceneId || applyingScenePos) return;
      try {
        const r = (await Editor.Message.request(
          'scene-editor',
          'query-spawn-preview-transforms'
        )) as {
          ok?: boolean;
          items?: Array<{ layerId: number; itemIndex: number; x: number; y: number; z: number }>;
        };
        ...
        for (const t of r.items) {
          ...
          const item = layer?.items?.[t.itemIndex ?? 0];
          if (!item) continue;
          ...
          for (const axis of ['x', 'y', 'z'] as const) {
            const inp = listEl.querySelector(
              `input[data-pos-layer="${t.layerId}"][data-pos-axis="${axis}"]`
            ) as HTMLInputElement | null;
            if (inp) inp.value = String(pos[axis]);
          }
        }
    ...
    const mkText = (value: string, ph: string) => {
      const inp = document.createElement('input');
      inp.type = 'text';
      inp.className = 'wide';
      inp.value = value;
    ...
    const appendRhythmReadonly = (host: HTMLElement, layerId: number) => {
    ...
    const ensureItem = (layer: Layer, preferArea: boolean): Item => {
      if (layer.items[0]) return layer.items[0];
      const pick = defaultPick();
      const item: Item = preferArea
        ? {
      ...
      if (pick) applyPick(item, pick.avatar, pick.config, preferArea);
      else if (preferArea) {
    ...
    const renderList = () => {
      ...
      usable.forEach((layer, i) => {
        ...
        const del = document.createElement('ui-button') as HTMLElement;
        del.textContent = '删除';
        del.className = 'c-actions';
        del.addEventListener('click', () => {
          const idx = bundle.layers.indexOf(layer);
          if (idx >= 0) bundle.layers.splice(idx, 1);
        ...
        val.title = resolved
          ...
          : '';
        const pickBtn = document.createElement('ui-button') as HTMLElement;
        ...
        if (area) {
          const count = mkNum(String(item.enemyCount ?? 1));
        ...
        const level = mkNum(String(item.level ?? 1));
        ...
        if (area) {
          ...
          scaleRow.appendChild(
            Object.assign(document.createElement('span'), { className: 'tag', textContent: '区域XZ' })
          );
          (['x', 'z'] as const).forEach((k) => {
    ...
    const addEntry = (kind: 'area' | 'point') => {
      ...
      if (!configOf(id)) missingLayerIds.add(id);
      renderList();
      schedulePreview();
    };

    const loadFromDisk = async (opts?: { openPrefab?: boolean }) => {
      await loadPlantables();
      const ctx = (await Editor.Message.request(
        'scene-editor',
        'query-spawn-editor-context'
      )) as {
        logicId?: number;
      ...
      if (!ctx?.logicId || ctx.error) {
        metaEl.textContent = ctx?.error || '未指定逻辑场景';
        return;
      }
      logicId = ctx.logicId;
      assetsSceneId = ctx.assetsSceneId || 0;
      logicName = ctx.name || '';
      bundle = ctx.monsterSpawn?.layers?.length
        ? {
            formatVersion: ctx.monsterSpawn.formatVersion ?? 1,
      ...
      renderList();
      startDragSync();
      const openPrefab = opts?.openPrefab === true;
      setTimeout(() => refreshScenePreview(openPrefab), openPrefab ? 400 : 200);
    };

    this.$.pickerClose?.addEventListener('click', () => hidePicker());
    pickerEl.addEventListener('click', (ev) => {
      if (ev.target === pickerEl) hidePicker();
    });
    ...
    this.$.btnAddArea?.addEventListener('click', () => addEntry('area'));
    this.$.btnAddPoint?.addEventListener('click', () => addEntry('point'));
    this.$.btnPreview?.addEventListener('click', () => refreshScenePreview(true));
    ...
    this.$.btnImport?.addEventListener('click', () => {
      void (async () => {
        const confirmed = await Editor.Dialog.warn(
          `将用资源场景 ${assetsSceneId} 的 Prefab EnemyBornInfo 覆盖当前种植，是否继续？`,
          { title: '从Prefab导入', buttons: ['取消', '导入'], default: 0, cancel: 0 }
        );
        const response =
          typeof confirmed === 'number'
            ? confirmed
            : (confirmed as { response?: number })?.response;
        ...
        if (!r?.ok) {
          await Editor.Dialog.error(r?.error || '导入失败', {
            title: '从Prefab导入',
            buttons: ['确定'],
            default: 0,
          });

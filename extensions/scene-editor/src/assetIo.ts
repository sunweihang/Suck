...
export async function writeTextAsset(dbUrl: string, content: string): Promise<boolean> {
  ...
  if (!existing) {
    try {
      ...
      console.warn('[scene-editor] create-asset failed, fallback fs write', dbUrl, e);
    }
  }
  ...
  try {
    ...
    console.error('[scene-editor] writeTextAsset failed', dbUrl, e);
    return false;
  }
}
...
export async function ensureAssetFolder(dbUrl: string, fsPath: string): Promise<void> {
  ...
  try {
    ...
    try {
      await Editor.Message.request('asset-db', 'refresh-asset', SCENES_PARENT);
    } catch {
      /* ignore */
    }
  }
...
const SCENES_PARENT = 'db://assets/resources/scenes';

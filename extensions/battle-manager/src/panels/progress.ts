...
function render(state: ProgressViewState | null): void {
  if (!rootEl || !state) return;
  const title = rootEl.querySelector('#prog-title') as HTMLElement | null;
  const label = rootEl.querySelector('#prog-label') as HTMLElement | null;
  const meta = rootEl.querySelector('#prog-meta') as HTMLElement | null;
  const fill = rootEl.querySelector('#prog-fill') as HTMLElement | null;
  const log = rootEl.querySelector('#prog-log') as HTMLElement | null;
  const pct = clampPct(state.current, state.total);

  if (title) title.textContent = state.title || '处理中…';
...
module.exports = Editor.Panel.define({
  ...
  methods: {
    panelProgressUpdate(state: ProgressViewState) {
      render(state);
    },
  },
  ready(this: { $: Record<string, HTMLElement | undefined> }) {
    ...
    void Editor.Message.request('battle-manager', 'query-progress')
      .then((state) => render(state as ProgressViewState | null))
      .catch(() => undefined);
  },
  close() {
    rootEl = null;

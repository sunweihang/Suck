...
function renderList(items) {
    ...
    for (const s of items) {
      ...
      <div class="meta">
        ...
        <div class="flags">${s.exportFlag ? '导出' : '未导出'} · ${s.hasGraph ? '有图' : '无图'}${s.category ? ` · ${escapeHtml(s.category)}` : ''}</div>
      </div>
      <div class="actions">
    ...
    listEl.querySelectorAll('button').forEach((btn) => {
        btn.addEventListener('click', async () => {
            ...
            if (act === 'edit') {
                ...
                const r = (0, validateBallisticGraph_1.validateBallisticOnDisk)(id);
                const msg = r.ok
                    ? `校验通过\n${r.warnings.join('\n')}`
                    : `校验失败\n${r.errors.join('\n')}\n${r.warnings.join('\n')}`;
                try {
...
async function refresh() {
    renderList((0, browseBallistics_1.listLocalBallistics)());
}

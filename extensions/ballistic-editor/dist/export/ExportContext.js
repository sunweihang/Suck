...
class ExportContext {
    ...
    findEntrance() {
        var _a;
        return (_a = this.graph.nodes.find((n) => n.typeName === ballisticNodes_1.ENTRANCE_TYPE)) !== null && _a !== void 0 ? _a : null;
    }
    collectReachable(entrance) {
        ...
        while (queue.length > 0) {
            ...
            if (!node || node.typeName === ballisticNodes_1.ENTRANCE_TYPE)
                continue;
            ...
            for (let i = 0; i < node.inputs.length; i++) {
                ...
                for (const c of this.inConns(node.id, i)) {
                    ...
                    if (!from || from.typeName === ballisticNodes_1.ENTRANCE_TYPE)
                        continue;
                    if (!seen.has(from.id)) {
                        seen.add(from.id);
                        queue.push(from.id);
    ...
    buildNodePlaceholders(info) {
        ...
        const fields = {};
        const def = (0, ballisticNodes_1.findBallisticNodeDef)(node.typeName);

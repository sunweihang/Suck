...
exports.openBallistic = openBallistic;
exports.load = load;
exports.unload = unload;
const browseBallistics_1 = require("./browseBallistics");
const createBallistic_1 = require("./createBallistic");
const TsBallisticExporter_1 = require("./export/TsBallisticExporter");
const findSkillsUsingBallistic_1 = require("./findSkillsUsingBallistic");
const ballisticNodes_1 = require("./nodes/ballisticNodes");
const ballisticPortTypes_1 = require("./nodes/ballisticPortTypes");
...
async function ensureRegistered(silent = false) {
    ...
    if (!(await ensureNodeGraph(silent)))
        return { ok: false };
    try {
        const nodes = (0, ballisticNodes_1.allBallisticRegisterNodes)();
        await Editor.Message.request(NODE_GRAPH, 'register-port-types', { portTypes: ballisticPortTypes_1.BALLISTIC_PORT_TYPES });
        await Editor.Message.request(NODE_GRAPH, 'register-nodes', { nodes });
        registered = true;
        console.log(`[ballistic-editor] registered ${nodes.length} ballistic nodes (${ballisticNodes_1.BALLISTIC_NODE_DEFS.length} domain + builtins)`);
        return { ok: true };
    }
    catch (e) {
        if (silent)
            console.warn('[ballistic-editor] register failed', e);
        else
            await dialogError(`注册弹道节点失败: ${e}`);
        return { ok: false };
    }
}
...
async function openBallistic(arg) {
    const ballisticId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.ballisticId;
    if (!ballisticId) {
    ...
    await Editor.Message.request(NODE_GRAPH, 'open-graph', {
        path: (0, paths_1.graphDbUrl)(ballisticId),
...
exports.methods = {
    ...
    async validateBallistic(arg) {
        const ballisticId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.ballisticId;
        if (!ballisticId)
            return { ok: false };
        const r = (0, validateBallisticGraph_1.validateBallisticOnDisk)(ballisticId);
        const msg = r.ok
            ? `校验通过\n${r.warnings.join('\n')}`
            : `校验失败\n${r.errors.join('\n')}\n${r.warnings.join('\n')}`;
        if (r.ok)
    ...
    async createBallistic(arg) {
        ...
        const result = await (0, createBallistic_1.createBallisticAssets)({
            ballisticId,
    ...
    async createBallisticApi(arg) {
        ...
        const ballisticId = arg === null || arg === void 0 ? void 0 : arg.ballisticId;
        if (!ballisticId || ballisticId <= 0) {
            return { ok: false, error: '请手动指定 ballisticId' };
        }
        return (0, createBallistic_1.createBallisticAssets)({
            ballisticId,
            name: (arg === null || arg === void 0 ? void 0 : arg.name) || `Ballistic ${ballisticId}`,
    ...
    async editBallisticGraph() {
        if (!(await ensureRegistered()).ok)
            return;
        const items = (0, browseBallistics_1.listLocalBallistics)();
        if (items.length === 0) {
    ...
    async openBallistic(arg) {
        return openBallistic(arg);
    },
    async listBallistics() {
        return (0, browseBallistics_1.listLocalBallistics)();
    },
    async validateBallisticGraph() {
        ...
        for (const s of items) {
            const r = (0, validateBallisticGraph_1.validateBallisticOnDisk)(s.ballisticId);
    ...
    async exportTs() {
        ...
        const r = (0, TsBallisticExporter_1.exportBallisticTs)(last.ballisticId);
        if (!r.ok) {
    ...
    async exportBallistic(arg) {
        const ballisticId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.ballisticId;
        if (!ballisticId)
            return { ok: false, error: 'missing ballisticId' };
        const r = (0, TsBallisticExporter_1.exportBallisticTs)(ballisticId);
        if (!r.ok)
            await dialogError(`导出失败: ${r.error}`);
        else
            await dialogInfo(`已导出 ${ballisticId}\n${r.path}`);
        return r;
    },
    async exportTsBatch() {
        const { ok, fail, results } = (0, TsBallisticExporter_1.exportAllFlagged)();
    ...
    async findSkillsUsingBallistic(arg) {
        const ballisticId = typeof arg === 'number' ? arg : arg === null || arg === void 0 ? void 0 : arg.ballisticId;
        if (!ballisticId)
            return [];
        const hits = (0, findSkillsUsingBallistic_1.findSkillsUsingBallistic)(ballisticId);

...
export function portTypeToTs(portType: string): string {
  switch (portType) {
    ...
    case 'vec3':
      return 'Vec3';
    case 'ballistic':
      return 'AbsLinearBallistic | null';
    default:
      return 'unknown';
  }
}
...
export class ExportContext {
  ...
  constructor(graph: NodeGraphJSON, ballisticId: number) {
    this.graph = graph;
    this.ballisticId = ballisticId;
  }
  ...
  buildNodePlaceholders(info: NodeVisitInfo): Record<string, string> {
    ...
    const def = findBallisticNodeDef(node.typeName);
    if (def?.fields) {

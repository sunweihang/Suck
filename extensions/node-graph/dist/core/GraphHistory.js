...
exports.GraphHistory = void 0;
const MAX_STACK = 80;
/** 连续属性编辑合并为一步撤销的时间窗（ms） */
const FIELD_COALESCE_MS = 1000;
function cloneGraph(json) {
...
class GraphHistory {
     ...
     */
    pushBefore(before, reason) {
        if (reason === 'load' || reason === 'undo' || reason === 'redo')
            return;
        const now = Date.now();
        const coalesce = reason === 'field' && this.lastReason === 'field' && now < this.coalesceUntil;
        if (!coalesce) {
...
exports.GraphHistory = GraphHistory;

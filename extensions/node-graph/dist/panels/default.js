...
const GraphCanvas_1 = require("../canvas/GraphCanvas");
const NodeCreator_1 = require("../canvas/NodeCreator");
const NodeDrawer_1 = require("../canvas/NodeDrawer");
const GraphProfile_1 = require("../core/GraphProfile");
...
function ensureLocalRegistry(defs, portTypes) {
    PortTypeRegistry_1.PortTypeRegistry.ensureInit();
    if (portTypes === null || portTypes === void 0 ? void 0 : portTypes.length)
        PortTypeRegistry_1.PortTypeRegistry.registerMany(portTypes);
    for (const d of defs)
        NodeRegistry_1.NodeRegistry.register(d);
    if (!NodeRegistry_1.NodeRegistry.get('FlowStart')) {
        NodeRegistry_1.NodeRegistry.registerMany(builtins_1.BUILTIN_NODES);
    }
}
async function syncRegistryFromMain() {
    const [defs, portTypes] = await Promise.all([
        Editor.Message.request(PKG, 'query-node-defs'),
        Editor.Message.request(PKG, 'query-port-types'),
    ]);
    ensureLocalRegistry(defs !== null && defs !== void 0 ? defs : [], portTypes !== null && portTypes !== void 0 ? portTypes : []);
}
function setDirty(v, panel) {
    dirty = v;
...
function broadcastChanged(reason) {
    var _a;
    Editor.Message.broadcast('node-graph:graph-changed', {
        path: currentPath,
        reason,
        graph: (_a = graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.graph.toJSON()) !== null && _a !== void 0 ? _a : null,
    });
}
function renderInspector(panel) {
    ...
    if (fields.length === 0) {
        ...
        for (const f of fields) {
            const val = (_b = (_a = node.customData[f.key]) !== null && _a !== void 0 ? _a : f.default) !== null && _b !== void 0 ? _b : '';
            ...
            if (f.type === 'bool') {
                ...
                html += `<input type="number" data-key="${escapeHtml(f.key)}" value="${escapeHtml(String(val))}" step="${(_c = f.step) !== null && _c !== void 0 ? _c : (f.type === 'int' ? 1 : 0.1)}"/>`;
            }
            else {
    ...
    const applyField = (el) => {
        ...
        if (el instanceof HTMLInputElement && el.type === 'checkbox') {
            ...
            value = (field === null || field === void 0 ? void 0 : field.type) === 'int' ? parseInt(el.value, 10) : parseFloat(el.value);
    ...
    const first = box.querySelector('input[type="number"],input[type="text"]');
...
function showCreator(panel, screenX, screenY, cx, cy) {
    addNodeCanvasPos = { x: cx, y: cy };
    const pop = panel.$.creator;
    const list = panel.$.creatorList;
    const input = panel.$.creatorSearch;
    if (!pop || !list || !input || !graphCanvas)
        return;
...
function hideCreator(panel) {
    const pop = panel.$.creator;
    if (pop)
        pop.style.display = 'none';
}
function refreshCreatorList(panel, query) {
    const list = panel.$.creatorList;
    if (!list || !graphCanvas)
        return;
    const items = (0, NodeCreator_1.filterNodeDefs)(NodeRegistry_1.NodeRegistry.list(), graphCanvas.graph.profile, query);
    if (items.length === 0) {
...
async function loadPayload(panel, payload) {
    await syncRegistryFromMain();
    let graph;
    if (payload.path) {
        const loaded = await (0, serialize_1.loadGraphFromDb)(payload.path);
        if (loaded) {
            graph = loaded;
            currentPath = payload.path;
        }
        else {
            graph = NodeGraph_1.NodeGraph.createEmpty(payload.profile);
            currentPath = payload.path;
        }
    }
    else if (payload.graph) {
        graph = NodeGraph_1.NodeGraph.fromJSON(payload.graph);
        currentPath = null;
    }
    else {
        graph = NodeGraph_1.NodeGraph.createEmpty(payload.profile);
    ...
    if (payload.profile) {
        graph.profile = GraphProfile_1.GraphProfile.fromJSON(payload.profile);
    }
    if (!graphCanvas)
        return;
...
async function doSave(panel, pathOverride) {
    if (!graphCanvas)
        return { ok: false };
    const target = pathOverride !== null && pathOverride !== void 0 ? pathOverride : currentPath;
    if (!target) {
        return doSaveAs(panel);
    }
    let ok = false;
    if (target.startsWith('db://')) {
        ok = await (0, serialize_1.saveGraphToDb)(target, graphCanvas.graph);
    }
    else {
        try {
            (0, serialize_1.saveGraphToFile)(target, graphCanvas.graph);
            ok = true;
        }
        catch (e) {
            console.error(e);
...
async function doSaveAs(panel) {
    var _a, _b;
    if (!graphCanvas)
        return { ok: false };
    // Prefer project assets folder via dialog if available; fallback to db path prompt
    try {
        const result = (await ((_b = (_a = Editor.Dialog) === null || _a === void 0 ? void 0 : _a.select) === null || _b === void 0 ? void 0 : _b.call(_a, {
            title: '保存节点图',
            type: 'file',
        ...
        if (!file)
            return { ok: false };
        (0, serialize_1.saveGraphToFile)(file, graphCanvas.graph);
        currentPath = file;
        setDirty(false, panel);
        Editor.Message.broadcast('node-graph:graph-saved', {
            path: currentPath,
        ...
        const ok = await (0, serialize_1.saveGraphToDb)(suggested, graphCanvas.graph);
        if (ok) {
            setDirty(false, panel);
            Editor.Message.broadcast('node-graph:graph-saved', {
                path: currentPath,
...
module.exports = Editor.Panel.define({
    listeners: {
        show() {
            graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.resize();
            graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.requestDraw();
        },
    },
    template: `
    <div class="root">
  ...
  `,
    ...
    methods: {
        async loadIntoPanel(payload) {
            await loadPayload(this, payload || {});
            return { ok: true, path: currentPath };
        },
        panelGetGraph() {
            var _a;
            return (_a = graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.graph.toJSON()) !== null && _a !== void 0 ? _a : null;
        },
        async panelSetGraph(arg) {
            await loadPayload(this, {
                graph: arg === null || arg === void 0 ? void 0 : arg.graph,
                profile: arg === null || arg === void 0 ? void 0 : arg.profile,
            });
            return { ok: true };
        },
        async panelSaveGraph(arg) {
            return doSave(this, arg === null || arg === void 0 ? void 0 : arg.path);
        },
        async panelRefreshRegistry() {
            await syncRegistryFromMain();
            if (graphCanvas) {
                for (const n of graphCanvas.graph.nodes)
                    (0, NodeDrawer_1.measureNode)(n);
                graphCanvas.requestDraw();
            }
            return { ok: true };
    ...
    async ready() {
        ...
        const panel = this;
        const info = (await Editor.Message.request(PKG, 'panel-ready'));
        ensureLocalRegistry((_a = info === null || info === void 0 ? void 0 : info.nodes) !== null && _a !== void 0 ? _a : builtins_1.BUILTIN_NODES, info === null || info === void 0 ? void 0 : info.portTypes);
        const canvasEl = panel.$.graphCanvas;
        graphCanvas = new GraphCanvas_1.GraphCanvas(canvasEl, NodeGraph_1.NodeGraph.createEmpty(), {
            onChange: (reason) => {
                if (reason !== 'load') {
                    setDirty(true, panel);
        ...
        graphCanvas.resize();
        resizeObserver = new ResizeObserver(() => graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.resize());
        if (panel.$.canvasHost)
            resizeObserver.observe(panel.$.canvasHost);
        (_b = panel.$.btnOpen) === null || _b === void 0 ? void 0 : _b.addEventListener('click', () => doOpen(panel));
        (_c = panel.$.btnSave) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => doSave(panel));
        (_d = panel.$.btnSaveAs) === null || _d === void 0 ? void 0 : _d.addEventListener('click', () => doSaveAs(panel));
        (_e = panel.$.btnDelete) === null || _e === void 0 ? void 0 : _e.addEventListener('click', () => {
            graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.deleteSelection();
            setDirty(true, panel);
            broadcastChanged('remove');
            renderInspector(panel);
        });
        (_f = panel.$.btnAdd) === null || _f === void 0 ? void 0 : _f.addEventListener('click', () => {
            if (!graphCanvas)
                return;
        ...
        const search = panel.$.creatorSearch;
        search === null || search === void 0 ? void 0 : search.addEventListener('input', () => refreshCreatorList(panel, search.value));
        search === null || search === void 0 ? void 0 : search.addEventListener('keydown', (e) => {
            if (e.key === 'Escape')
                hideCreator(panel);
        });
        ...
        document.addEventListener('pointerdown', (e) => {
            ...
            if (path.includes(pop) || pop.contains(e.target))
                return;
            hideCreator(panel);
        });
        setDirty(false, panel);
    ...
    close() {
        ...
        resizeObserver = null;
        graphCanvas === null || graphCanvas === void 0 ? void 0 : graphCanvas.destroy();
        graphCanvas = null;

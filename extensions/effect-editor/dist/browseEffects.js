...
exports.listLocalEffects = listLocalEffects;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const paths_1 = require("./paths");
function listLocalEffects() {
    ...
    for (const name of fs.readdirSync(root)) {
        ...
        items.push({
            ...index,
            hasPrefab,
            subtitle: `${index.poolName || ''} · ${index.prefab || '(无 prefab)'}${hasPrefab ? '' : index.prefab ? ' · 缺Prefab' : ''}`,
        });
    }
    items.sort((a, b) => a.effectId - b.effectId);
    return items;
}
//# sourceMappingURL=browseEffects.js.map
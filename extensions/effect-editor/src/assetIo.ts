...
const EFFECTS_PARENT = 'db://assets/resources/effects';
...
export async function writeTextAsset(dbUrl: string, content: string): Promise<boolean> {
  ...
  if (!existing) {
    try {
      ...
      console.warn('[effect-editor] create-asset failed, fallback fs write', dbUrl, e);
    }
  }
  ...
  try {
    ...
    console.error('[effect-editor] writeTextAsset failed', dbUrl, e);
    return false;
  }
}

export async function ensureAssetFolder(dbUrl: string, fsPath: string): Promise<void> {
  ensureDir(fsPath);
  try {
    await Editor.Message.request('asset-db', 'refresh-asset', dbUrl);
  } catch {
    try {
      await Editor.Message.request('asset-db', 'refresh-asset', EFFECTS_PARENT);
    } catch {
      /* ignore */
    }
  }

...
import { listLocalEffectCategories } from './browseCategories';
import { listLocalEffects } from './browseEffects';
import { createEffectAssets } from './createEffect';
import { deleteEffectAssets } from './deleteEffect';
import { migrateAllEffects } from './migrateEffects';
import { effectFolderDbUrl, prefabDbUrl } from './paths';
import { validateEffectOnDisk } from './validateEffect';
...
async function openInGameEditor(
  moduleId: 'effect' | 'effect-category' = 'effect'
): Promise<void> {
  try {
...
export async function openEffect(arg: { effectId: number } | number): Promise<{ ok: boolean }> {
  const effectId = typeof arg === 'number' ? arg : arg?.effectId;
  if (!effectId) {
...
export const methods = {
  ...
  async listEffects() {
    return listLocalEffects();
  },

  async listEffectCategories() {
    return listLocalEffectCategories();
  },

  async openEffect(arg: { effectId: number } | number) {
    return openEffect(arg);
  },

  async openEffectCategory(arg: { categoryId: number } | number) {
    const categoryId = typeof arg === 'number' ? arg : arg?.categoryId;
    const cats = listLocalEffectCategories();
    const cat = cats.find((c) => c.categoryId === categoryId);
    if (!cat) {
      await dialogWarn('类型不存在');
      return { ok: false };
    }
    const effects = listLocalEffects().filter(
      (e) => (e.category || 'uncategorized') === cat.name
    );
    const lines = effects.map(
      (e) => `· ${e.effectId} ${e.name} (${e.poolName || e.prefab || '无'})`
    );
    await dialogInfo(`类型「${cat.name}」共 ${effects.length} 个特效\n${lines.join('\n') || '(空)'}`);
    return { ok: true, category: cat.name, effects };
  },

  async createEffect(arg?: { effectId?: number; poolName?: string } | number) {
    const effectId = typeof arg === 'number' ? arg : arg?.effectId;
    if (!effectId || effectId <= 0) {
      Editor.Panel.open(`${PKG}.create`);
      return { ok: false, cancelled: true };
    }
    const poolName = typeof arg === 'object' && arg?.poolName ? arg.poolName : undefined;
    const result = await createEffectAssets({ effectId, poolName });
    if (!result.ok) {
  ...
  async createEffectApi(arg: {
    ...
    const effectId = arg?.effectId;
    if (!effectId || effectId <= 0) {
      return { ok: false, effectId: 0, error: '请手动指定 effectId' };
    }
    return createEffectAssets({
      effectId,
      name: arg?.name,
  ...
  async deleteEffect(arg: { effectId: number } | number) {
    const effectId = typeof arg === 'number' ? arg : arg?.effectId;
    if (!effectId) {
      await dialogWarn('请提供 effectId');
      return { ok: false };
    ...
    const confirmed = await dialogConfirm(
      `确定删除特效 ${label}？\n将删除 assets/resources/effects/${effectId}/（不可恢复）`,
      '删除'
    );
    if (!confirmed) return { ok: false, cancelled: true };

    const result = await deleteEffectAssets(effectId);
    if (!result.ok) {
  ...
  async locateEffect(arg: { effectId: number } | number) {
    const effectId = typeof arg === 'number' ? arg : arg?.effectId;
    if (!effectId) {
      await dialogWarn('请提供 effectId');
      return { ok: false };
    ...
    const dbUrl = item.prefab ? prefabDbUrl(item.prefab) : effectFolderDbUrl(effectId);
    try {
  ...
  async validateEffect(arg: { effectId: number } | number) {
    const effectId = typeof arg === 'number' ? arg : arg?.effectId;
    if (!effectId) return { ok: false };
    const r = validateEffectOnDisk(effectId);
    const msg = r.ok
      ? `校验通过\n${r.warnings.join('\n')}`
      : `校验失败\n${r.errors.join('\n')}\n${r.warnings.join('\n')}`;
  ...
  async validateEffects() {
    const effects = listLocalEffects();
    if (effects.length === 0) {
    ...
    for (const e of effects) {
      const r = validateEffectOnDisk(e.effectId);
  ...
  async migrateEffects() {
    const confirmed = await dialogConfirm(
      '将 Prefabs/SFX_* 与 Prefabs/VFX_* 迁移到 assets/resources/effects/{id}/Output/\n' +
        '并更新 res.json url（name / CreatNode 名不变）。\n\n共享美术 Art/SFX 不会移动。\n是否继续？',
      '迁移'
    );
    if (!confirmed) return { ok: false, cancelled: true };

    const r = await migrateAllEffects();
    await dialogInfo(
